...
@section('content')
    <div id="page-wrapper">
        <div class="row">
            ...
            <form class="edit_announcement" id="edit_slider_8" enctype="multipart/form-data"
                  action="/announcement/{{ $announcement->id }}/edit"
                                    ...
                                    @if (isset($slider->featured_file_name) && !empty($slider->featured_file_name))
                                        ...
                                        <div class="panel-footer">
                                            <a class="btn btn-default"
                                               href="/announcement/{{ $announcement->id }}/remove_photo">Remove
                                                Featured Photo</a>
                            ...
                            @if(Auth::user()->role == 'super'  || Auth::user()->role == 'admin')
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="status" id="status">
                                        <option @if(old('status',$announcement->status) == 0) selected="selected" @endif value="0">Draft</option>
                                        <option @if(old('status',$announcement->status) == 1) selected="selected" @endif value="1">Published</option>
                                        <option @if(old('status',$announcement->status) == 2) selected="selected" @endif value="2">Archived</option>
                                    </select>
                                </div>
                            @endif
...
value="@if(!empty(old('start_publishing',$announcement->start_publishing)))
{{ old('start_publishing',\Carbon\Carbon::parse($announcement->start_publishing)->format('d-m-Y')) }}@endif" />
                                    ...
                                    <div class="col-md-12">
                                        <div class='input-group date' id='datetimepicker2'>
                                            <input type='text' class="form-control" name="stop_publishing"
                                                   value="@if (!empty(old('stop_publishing',$announcement->stop_publishing))){{ old('stop_publishing',\Carbon\Carbon::parse($announcement->stop_publishing)->format('d-m-Y')) }}@endif" />
                                            <span class="input-group-addon">
                                                <span class="glyphicon glyphicon-calendar"></span>
                                                </span>
                                        </div>
